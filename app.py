...
<style>
    ...
    div[data-testid="stMetricValue"] {
        font-size: 28px;
        font-weight: 700;
        color: #2563EB !important; /* Royal Accent Blue */
    }

    div[data-testid="stMetricLabel"] {
        color: #94A3B8 !important; /* Cool Muted Slate */
        font-weight: 500;
    }
    ...
    .ai-report {
        background-color: #1E293B;
        border: 1px solid #2563EB; /* Glowing Blue Accent Border */
        border-radius: 10px;
        padding: 24px;
        margin-top: 10px;
        line-height: 1.8;
    ...
    .sub-header {
        color: #38BDF8; /* Bright Blue Sub-header */
        font-size: 15px;
        margin-bottom: 20px;
        font-weight: 500;
    }
...
def run_inference(transaction_data):
    ...
    if model is None or preprocessor is None:
        ...
        st.error(
            "ML model or preprocessor was not found."
        )
    ...
    try:
        ...
        if (
            transaction_data["ip_risk_score"] > 0.60
            or (
...
def generate_insights(transaction):
    ...
    if transaction.get(
        ...
        flags.append(
            f"Suspicious IP risk score "
            f"({transaction['ip_risk_score']:.2f})"
        )

    if (
        transaction.get("is_foreign_txn") == 1
        and transaction.get("has_2fa") == 0
    ):

        flags.append(
            "Cross-border transaction attempted "
            "without 2FA"
        )
    ...
    if transaction.get(
        ...
        flags.append(
            f"Unusually high transaction ratio "
            f"({transaction['amount_vs_avg_ratio']}x baseline)"
        )
    ...
    if transaction.get(
        ...
        flags.append(
            ...
            f"({transaction['velocity_1h']} transactions "
            f"in the last hour)"
        )
    ...
    if not flags:
        ...
        return (
            "Transaction matches expected spending "
            "patterns. No major anomaly flags were triggered."
        )
...
# ============================================================
    ...
    st.caption(
        "Enter transaction information to perform an ML fraud assessment."
    )
    ...
    if st.button(
        ...
        with st.spinner(
            "ML model analyzing transaction..."
        ):
        ...
        st.success(
            "Transaction assessment completed successfully."
        )
    ...
    if st.session_state.last_result:
        ...
        if result["risk"] == "HIGH":
            ...
            st.error(
                ...
                "or route it for manual verification."
            )

        elif result["risk"] == "MEDIUM":

            st.warning(
                "⚠️ MEDIUM RISK — Request additional "
...
# ============================================================
    ...
    st.caption(
        "ML prediction explained by the Ollama-powered AI fraud agent."
    )
    ...
    if not st.session_state.last_result:
        ...
        st.info(
            "No transaction has been evaluated yet."
        )

        st.write(
            "Go to **Evaluate Risk**, enter the transaction "
            "details and click **Run Security Assessment**."
        )
        ...
        if st.button(
            ...
            with st.spinner(
                "AI Agent is analyzing the transaction..."
            ):

                try:

                    # Import the actual working AI agent
                    from agent.fraud_agent import (
                        Transaction,
                        fraud_check
                    )

                    # Convert Streamlit dictionary
                    # into Pydantic Transaction object
                    ...
                    # Store ML response
                    st.session_state.agent_ml_result = (
                        result["ml_prediction"]
                    )

                    st.success(
                        "AI diagnostic report generated successfully."
                    )
        ...
        if st.session_state.agent_notes:
            ...
            if st.session_state.get(
                ...
                with m2:
                    ...
                    st.metric(
                        "Fraud Probability",
                        f"{ml_result['fraud_probability'] * 100:.6f}%"
                    )
                ...
                st.info(
                    "The ML model determines the fraud probability. "
                    "The AI agent uses that result to generate an "
...
# ============================================================
    ...
    st.caption(
        "Session-level analytics for evaluated transactions."
    )
    ...
    if not st.session_state.history:
        ...
        st.info(
            "No transactions have been evaluated yet."
        )
